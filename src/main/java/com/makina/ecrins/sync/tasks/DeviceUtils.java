...
public class DeviceUtils
{
	...
	public static String getDefaultExternalStorageDirectory(final DeviceSettings deviceSettings)
	{
		...
		{
			return deviceSettings.getMountsSettings().getDefaultMountPoint();
		}
	}
	...
	public static String getExternalStorageDirectory(final DeviceSettings deviceSettings)
	{
		...
		else
		{
			...
			if (StringUtils.isBlank(deviceSettings.getMountsSettings().getExternalMountPoint()))
			{
				return deviceSettings.getMountsSettings().getDefaultMountPoint();
			}
			
			return deviceSettings.getMountsSettings().getExternalMountPoint();
		}
	}
	...
	public static DeviceSettings findLoadedDeviceSettings(final DeviceSettings deviceSettings)
	{
		...
		if (LoadSettingsCallable.getInstance().getSettings().getDevicesSettings().contains(deviceSettings))
		{
			return LoadSettingsCallable.getInstance().getSettings().getDevicesSettings().get(
					(LoadSettingsCallable.getInstance().getSettings().getDevicesSettings().indexOf(deviceSettings)));
		}
